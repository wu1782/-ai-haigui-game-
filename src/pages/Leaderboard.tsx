      ...
      <main className="relative z-10 max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="space-y-3">
          {isLoading ? (
            <div className="text-center py-16 bg-white/80 dark:bg-dark-800/80 backdrop-blur-xl rounded-3xl border border-gray-200/50 dark:border-dark-700/50">
              <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-game-500/20 to-purple-500/20 flex items-center justify-center">
                <span className="text-3xl animate-pulse">🏆</span>
              </div>
              <p className="text-gray-500 dark:text-gray-400 font-medium">加载排行榜中...</p>
            </div>
          ) : (
            leaderboardData.map((entry, index) => (
              <div
                ...
                className={`flex items-center gap-4 p-4 sm:p-5 rounded-2xl border transition-all duration-200
                  hover:scale-[1.01] hover:shadow-lg ${getRankStyle(entry.rank)}`}
              >
                {/* 排名 */}
        ...
        {!isLoading && leaderboardData.length === 0 && (
          <div className="text-center py-16 bg-white/80 dark:bg-dark-800/80 backdrop-blur-xl rounded-3xl border border-gray-200/50 dark:border-dark-700/50">
            ...
            <p className="text-gray-500 dark:text-gray-400 font-medium">暂无排行榜数据</p>
            <p className="text-sm text-gray-400 dark:text-gray-500 mt-2">成为第一个上榜的玩家吧！</p>
          </div>
        )}
